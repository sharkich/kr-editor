...
} from '../../ANTLR/KRLangGrammarParser';
import { parseAndGetASTRoot, parseAndGetSyntaxErrors } from './parser';
import { IKRLangError } from './KRLangErrorListener';

const checkSemanticRules = (ast: TodoExpressionsContext): IKRLangError[] => {
  const errors: IKRLangError[] = [];
...
export class KRLangLanguageService {
  validate(code: string): IKRLangError[] {
    const syntaxErrors: IKRLangError[] = parseAndGetSyntaxErrors(code);
